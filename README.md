...
+ В параметрах `"project": "${JOB_BASE_NAME}"` и `"reportLink": "${BUILD_URL}"` - в качестве значений указываем переменные, в уведомлении здесь будут указаны название `JOB` и ссылка на `BUILD` в Jenkins.
